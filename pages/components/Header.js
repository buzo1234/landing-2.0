...
function Header() {
  ...
  useEffect(() => {
    if (cartstate) {
      setSubqty(
        cartstate.cart.reduce((qty, item) => Number(item.qty) + qty, 0)
      );
    }
  }, [cartstate]);
  return (
        ...
        <div className='flex items-center justify-center'>
          ...
          <Link href='/cartarea'>
            <div className='relative w-full cursor-pointer '>
              <div className='absolute -mt-1 ml-9 justify-center bg-red-400 items-center text-xs text-center rounded-full w-4'>
                {subqty}
              </div>
